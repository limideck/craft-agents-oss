...
import { generateCallbackPage } from '@grose-agent/shared/auth'
...
export interface WebuiHandlerOptions {
  ...
  webuiDir: string
  /** Secret used to sign JWTs — typically GROSE_SERVER_TOKEN. */
  secret: string
...
export function createWebuiHandler(options: WebuiHandlerOptions): WebuiHandler {
  ...
  async function fetch(req: Request): Promise<Response> {
    ...
    if (path === '/api/config/workspaces' && req.method === 'GET') {
      ...
      const { getActiveWorkspace } = await import('@grose-agent/shared/config/storage')
