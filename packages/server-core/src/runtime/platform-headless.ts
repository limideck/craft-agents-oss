...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.GROSE_DEBUG === 'true' || process.env.GROSE_IS_PACKAGED !== 'true') {
 ...
 * - GROSE_APP_ROOT — override appRootPath (default: cwd)
 * - GROSE_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - GROSE_IS_PACKAGED — 'true' for production (default: false)
 * - GROSE_VERSION — app version string (default: '0.0.0-dev')
 * - GROSE_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.GROSE_DEBUG === 'true' || process.env.GROSE_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.GROSE_APP_ROOT || process.cwd(),
    resourcesPath: process.env.GROSE_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.GROSE_IS_PACKAGED === 'true',
    appVersion: process.env.GROSE_VERSION || options?.appVersion || '0.0.0-dev',
