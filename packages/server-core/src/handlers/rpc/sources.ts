...
import { loadWorkspaceSources } from '@grose-agent/shared/sources'
import { safeJsonParse } from '@grose-agent/shared/utils/files'
...
export function registerSourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Create a new source
  server.handle(RPC_CHANNELS.sources.CREATE, async (_ctx, workspaceId: string, config: Partial<import('@grose-agent/shared/sources').CreateSourceInput>) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createSource } = await import('@grose-agent/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.DELETE, async (_ctx, workspaceId: string, sourceSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deleteSource } = await import('@grose-agent/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.SAVE_CREDENTIALS, async (_ctx, workspaceId: string, sourceSlug: string, credential: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { loadSource, getSourceCredentialManager } = await import('@grose-agent/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.GET_PERMISSIONS, async (_ctx, workspaceId: string, sourceSlug: string) => {
    ...
    const { getSourcePermissionsPath } = await import('@grose-agent/shared/agent')
  ...
  server.handle(RPC_CHANNELS.workspace.GET_PERMISSIONS, async (_ctx, workspaceId: string) => {
    ...
    const { existsSync, readFileSync } = await import('fs')
    const { getWorkspacePermissionsPath } = await import('@grose-agent/shared/agent')
  ...
  // Get default permissions from ~/.grose-agent/permissions/default.json
  server.handle(RPC_CHANNELS.permissions.GET_DEFAULTS, async () => {
    const { existsSync, readFileSync } = await import('fs')
    const { getAppPermissionsDir } = await import('@grose-agent/shared/agent')
  ...
  server.handle(RPC_CHANNELS.sources.GET_MCP_TOOLS, async (_ctx, workspaceId: string, sourceSlug: string) => {
    ...
    try {
      ...
      const { GroseMcpClient } = await import('@grose-agent/shared/mcp')
      let client: InstanceType<typeof GroseMcpClient>
      ...
      const { loadSourcePermissionsConfig, permissionsConfigCache } = await import('@grose-agent/shared/agent')
