...
import { getCredentialManager, SOURCE_CREDENTIAL_TYPES } from '@grose-agent/shared/credentials'
...
} from '@grose-agent/shared/resources'
...
export function registerResourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, options: ExportResourcesOptions) => {
      ...
      const { exportResources } = await import('@grose-agent/shared/resources')
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, bundle: ResourceBundle, mode: ResourceImportMode) => {
      ...
      const { importResources } = await import('@grose-agent/shared/resources')
