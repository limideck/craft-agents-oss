...
import { ensureConfigDir, loadStoredConfig, saveConfig } from '@grose-agent/shared/config'
import { CONFIG_DIR } from '@grose-agent/shared/config/paths'
        ...
        if (lock.pid === process.pid) {
          ...
          if (isLockFromPreviousBoot(lock.startedAt)) {
            ...
            throw new Error(
              ...
              `To run a parallel instance (e.g. for dev), set GROSE_CONFIG_DIR to a different path.`
            )
          }
        } else {
...
export async function bootstrapServer<TSessionManager, THandlerDeps>(
  ...
  const serverToken = options.serverToken ?? process.env.GROSE_SERVER_TOKEN
  if (!serverToken) {
    throw new Error('Server token is required. Pass options.serverToken or set GROSE_SERVER_TOKEN.')
  }
  ...
  const bundledAssetsRoot = options.bundledAssetsRoot
    ?? process.env.GROSE_BUNDLED_ASSETS_ROOT
    ?? process.cwd()
  ...
  const rpcHost = options.rpcHost ?? process.env.GROSE_RPC_HOST ?? '127.0.0.1'
  const rpcPortRaw = options.rpcPort ?? parseInt(process.env.GROSE_RPC_PORT ?? '9100', 10)
  ...
  platform.logger.info(`Grose Agent server listening on ${wsServer.protocol}://${rpcHost}:${wsServer.port}`)
