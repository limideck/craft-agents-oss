...
describe('sdk-bridge', () => {
  describe('buildEnvFromSdkInput', () => {
    it('should always include GROSE_EVENT', () => {
      const env = buildEnvFromSdkInput('PreToolUse', input());
      expect(env.GROSE_EVENT).toBe('PreToolUse');
    });
    ...
    describe('PreToolUse / PostToolUse', () => {
      it('should map tool_name to GROSE_TOOL_NAME', () => {
        const env = buildEnvFromSdkInput('PreToolUse', input({ tool_name: 'Bash' }));
        expect(env.GROSE_TOOL_NAME).toBe('Bash');
      });
      ...
      it('should map tool_input as sanitized JSON', () => {
        ...
        expect(env.GROSE_TOOL_INPUT).toBeDefined();
        expect(env.GROSE_TOOL_INPUT).not.toContain('`');
      });
      ...
      it('should map tool_response for PostToolUse', () => {
        ...
        expect(env.GROSE_TOOL_RESPONSE).toBeDefined();
      });
    });

    describe('PostToolUseFailure', () => {
      it('should map error to GROSE_ERROR', () => {
        const env = buildEnvFromSdkInput('PostToolUseFailure', input({
          tool_name: 'Bash',
          error: 'Command failed',
        }));
        expect(env.GROSE_TOOL_NAME).toBe('Bash');
        expect(env.GROSE_ERROR).toBeDefined();
      });
    });
    ...
    describe('UserPromptSubmit', () => {
      it('should sanitize user prompt', () => {
        ...
        expect(env.GROSE_PROMPT).toBeDefined();
        // Backticks should be escaped with backslash
        expect(env.GROSE_PROMPT).toContain('\\`');
      });
    });
    ...
    describe('SessionStart', () => {
      it('should map source and model', () => {
        ...
        expect(env.GROSE_SOURCE).toBe('manual');
        expect(env.GROSE_MODEL).toBe('claude-opus-4-7');
      });
    });
    ...
    describe('SubagentStart / SubagentStop', () => {
      it('should map agent_id and agent_type', () => {
        ...
        expect(env.GROSE_AGENT_ID).toBe('agent-123');
        expect(env.GROSE_AGENT_TYPE).toBe('research');
      });
    });
    ...
    describe('Notification', () => {
      it('should sanitize message and title', () => {
        ...
        expect(env.GROSE_MESSAGE).toBeDefined();
        expect(env.GROSE_TITLE).toBeDefined();
        // Backticks should be escaped
        expect(env.GROSE_MESSAGE).toContain('\\`');
        expect(env.GROSE_TITLE).toContain('\\`');
      });
    });
    ...
    describe('unknown/default events', () => {
      it('should return minimal env for events with no specific mappings', () => {
        ...
        expect(env.GROSE_EVENT).toBe('Stop');
