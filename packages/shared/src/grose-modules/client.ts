/**
 * Loopback HTTP client for grose-modules RSS + Workflows + Sites APIs.
 *
 * Persistence resolves workspaceId → absolute rootPath (registry), then
 * reads/writes under `{rootPath}/modules/...`. Always send both
 * X-Grose-Workspace-Id and X-Grose-Workspace-Root when root is known.
 */

import { resolveWorkspaceRootPath } from '../config/storage.ts'
import { requireGroseModulesEndpoint } from './endpoint.ts'
import type {
...
export class GroseModulesHttpError extends Error {
  constructor(
    ...
    super(message)
    this.name = 'GroseModulesHttpError'
  }
}
...
function workspaceHeaders(workspaceId: string): Record<string, string> {
  const headers: Record<string, string> = {
    ...
    'X-Grose-Workspace-Id': workspaceId,
  }
  const root = resolveWorkspaceRootPath(workspaceId)
  if (root) {
    headers['X-Grose-Workspace-Root'] = root
  }
  return headers
}
...
async function request<T>(
  ...
  const ep = requireGroseModulesEndpoint()
  ...
  if (!res.ok) {
    ...
    throw new GroseModulesHttpError(message || `HTTP ${res.status}`, res.status, text)
  }
...
export async function listFeeds(workspaceId: string): Promise<GroseModulesRssFeed[]> {
...
export async function importOpml(
  workspaceId: string,
  opml: string,
): Promise<{ imported: number; skipped: number; feeds: GroseModulesRssFeed[] }> {
...
): Promise<{ articles: GroseModulesRssArticle[]; cacheReady?: boolean; query?: string }> {
...
export async function getArticle(
  workspaceId: string,
  articleId: string,
): Promise<GroseModulesRssArticle> {
...
export async function listWorkflows(workspaceId: string): Promise<GroseModulesWorkflow[]> {
...
export async function deployWorkflow(
  workspaceId: string,
  workflowId: string,
): Promise<GroseModulesWorkflowDeployResult> {
...
export async function listSites(workspaceId: string): Promise<GroseModulesSite[]> {
  return request(workspaceId, 'GET', '/api/sites')
}

export async function getSite(workspaceId: string, siteId: string): Promise<GroseModulesSite> {
  return request(workspaceId, 'GET', `/api/sites/${encodeURIComponent(siteId)}`)
}

export async function createSite(
  workspaceId: string,
  input: GroseModulesSiteCreateInput,
): Promise<GroseModulesSite> {
...
export async function listSiteFiles(
  workspaceId: string,
  siteId: string,
): Promise<GroseModulesSiteFileNode[]> {
...
export async function startSitePreview(
  workspaceId: string,
  siteId: string,
): Promise<GroseModulesSitePreviewResult> {
...
export async function saveSiteVisualEdit(
  workspaceId: string,
  input: GroseModulesVisualEditSaveInput,
): Promise<{ ok: true }> {
  return request(
    workspaceId,
    'POST',
