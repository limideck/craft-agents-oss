...
describe('unified-network-interceptor SSE processors', () => {
  ...
  beforeAll(async () => {
    process.env.GROSE_INTERCEPTOR_DISABLE_AUTO_INSTALL = '1';
  ...
  afterAll(() => {
    delete process.env.GROSE_INTERCEPTOR_DISABLE_AUTO_INSTALL;
  });
  ...
  it('OpenAI Responses: strips metadata on function_call done events', async () => {
    const sse = [
      ...
      'data: {"type":"response.output_item.done","item":{"type":"function_call","call_id":"call_resp_1","id":"fc_1","name":"mcp__grose__search","arguments":"{\\"foo\\":1,\\"_intent\\":\\"do thing\\",\\"_displayName\\":\\"Do Thing\\"}"}}\n\n',
      'data: [DONE]\n\n',
    ];
