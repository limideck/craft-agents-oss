...
function browserDocPath(): string {
  return resolve(join(homedir(), '.grose-agent', 'docs', 'browser-tools.md'));
}
...
describe('PrerequisiteManager', () => {
  ...
  describe('rule matching', () => {
    ...
    it('exempts grose-agents-docs MCP tools', () => {
      mockExistsPaths.add(guidePath('grose-agents-docs'));
      const result = manager.checkPrerequisites('mcp__grose-agents-docs__search');
      expect(result.allowed).toBe(true);
    });
