...
import type { AuthRequest } from '@grose-agent/session-tools-core';
...
export interface SessionScopedToolCallbacks {
  ...
  getSessionInfoFn?: (sessionId?: string) => import('@grose-agent/session-tools-core').SessionInfo | null;
  /** List sessions in the workspace with pagination. */
  listSessionsFn?: (options?: import('@grose-agent/session-tools-core').ListSessionsOptions) => import('@grose-agent/session-tools-core').ListSessionsResult;
  /** List background tasks (running + terminal) for a session from the main-process registry. */
  listBackgroundTasksFn?: (sessionId?: string) => import('@grose-agent/session-tools-core').BackgroundTaskInfo[];
  /** Resolve label display names to IDs. */
  resolveLabelsFn?: (labels: string[]) => import('@grose-agent/session-tools-core').ResolvedLabelsResult;
  /** Resolve a status display name to its ID. */
  resolveStatusFn?: (status: string) => import('@grose-agent/session-tools-core').ResolvedStatusResult;
  /** Send a message to another session (inter-session messaging). Resolves with delivery status. */
  sendAgentMessageFn?: (sessionId: string, message: string, attachments?: Array<{ path: string; name?: string }>) => Promise<import('@grose-agent/session-tools-core').SendAgentMessageResult>;
  /**
