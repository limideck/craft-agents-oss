 ...
 *
 * File locations:
 * - Workspace: ~/.grose-agent/workspaces/{slug}/permissions.json
 * - Per-source: ~/.grose-agent/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.grose-agent/permissions/
 * Reads env var dynamically so tests can override via GROSE_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.GROSE_CONFIG_DIR || join(homedir(), '.grose-agent');
...
/**
 * Load default permissions from ~/.grose-agent/permissions/default.json
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.groseAgentsCli && pattern.startsWith('^grose-agent\\s')) {
    return false;
  }
  return true;
}
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.grose-agent/permissions/default.json)
   ...
   * These come from ~/.grose-agent/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping grose-agent bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
