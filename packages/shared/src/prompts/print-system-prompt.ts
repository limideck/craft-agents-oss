#!/usr/bin/env bun
/**
 * Debug script to print the full Grose Agent system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    GROSE AGENT SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. Grose Agent Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.grose-agent/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@grose-agent/electron/main.log' },
  '/Users/example/.grose-agent/workspaces/abc123'
);
...
printAnnotation('Generated by GroseAgent.formatSourceState() - requires agent instance');
...
// 4b. Grose modules prefer-builtin catalog
const exampleGroseModules = `<grose_modules>
Prefer builtin Grose modules over creating new API/MCP Sources when user intent matches.
Source slug: grose-modules
…
</grose_modules>`;
printSection(
  '4b. GROSE MODULES - formatGroseModulesContext() [STABLE]',
  exampleGroseModules,
  colors.magenta
);
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Grose Agent additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Grose Agent Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Dynamic User Message Components (per message):${colors.reset}
  ...
  4b. Grose Modules catalog              ${colors.dim}// formatGroseModulesContext()      [STABLE]${colors.reset}
  4c. Grose Modules active (when set)    ${colors.dim}// formatGroseModulesActiveLine()   [VOLATILE]${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/grose-agent.ts       ${colors.dim}// User message building${colors.reset}
