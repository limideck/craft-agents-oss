...
export function prepareSlackOAuth(options: PrepareSlackOAuthOptions): PreparedOAuthFlow {
  ...
  const redirectUri = options.callbackUrl
    ?? `https://agents.grose.do/auth/slack/callback?port=${options.callbackPort}`;
...
export async function startSlackOAuth(options: SlackOAuthOptions = {}): Promise<SlackOAuthResult> {
  try {
    ...
    // The relay redirects: https://agents.grose.do/auth/slack/callback → http://localhost:{port}/callback
    const redirectUri = `https://agents.grose.do/auth/slack/callback?port=${port}`;
