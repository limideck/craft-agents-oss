...
describe('buildOAuthDeeplinkUrl', () => {
  it('returns deeplink URL when sessionId and deeplinkScheme are provided', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'groseagents',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('groseagents://allSessions/session/260209-swift-river');
  });
  ...
  it('uses custom deeplink scheme for multi-instance', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'groseagents1',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('groseagents1://allSessions/session/test-session');
  });
  ...
  it('handles session ID with special characters (hyphens and numbers)', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('groseagents://allSessions/session/260209-swift-river-42');
  });
  ...
  it('handles session ID with URL-unsafe characters', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('groseagents://allSessions/session/session/with spaces&special=chars');
  });
  ...
  it('constructs correct URL format for instance 2', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'groseagents2',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('groseagents2://allSessions/session/260111-bold-moon');
  });
});
