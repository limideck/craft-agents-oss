...
/**
 * Validates an HTTP/SSE MCP connection by connecting via GroseMcpClient and
...
export async function validateMcpConnection(
  ...
  // SSE transport is not supported by GroseMcpClient (HTTP only). Streamable
  // HTTP is the modern transport; SSE servers will surface a clear connect error.
  const mcpClient = new GroseMcpClient({
    transport: 'http',
    url: mcpUrl,
...
export async function validateStdioMcpConnection(
  ...
  try {
    ...
    client = new Client(
      { name: 'grose-agent-validator', version: '1.0.0' },
      { capabilities: {} }
    );
