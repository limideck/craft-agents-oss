...
export async function installArchive(params: { archiveData: ArrayBuffer, version: string }): Promise<void> {
  ...
  const versionDirectory = join(homedir(), '.local', 'share', 'grose', 'versions', version);
  const binaryPath = join(versionDirectory, 'grose');
  const symlinkDirectory = join(homedir(), '.local', 'bin');
  const symlinkPath = join(symlinkDirectory, 'grose');
