 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.grose-agent/workspaces/xxx)
 ...
 * (like grose-agents-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only grose-agents-docs is a builtin source
      if (slug === 'grose-agents-docs') {
 ...
 * Built-in sources (like grose-agents-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search grose-agents-docs MCP for service-specific guidance)
