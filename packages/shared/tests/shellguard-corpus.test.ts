...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // grose-agent CLI read-only
    { regex: /^grose-agent\s+label\s+(list|get)\b/, source: '^grose-agent\\s+label\\s+(list|get)\\b', comment: 'grose-agent label read-only operations' },
    { regex: /^grose-agent\s+source\s+(list|get|validate|test)\b/, source: '^grose-agent\\s+source\\s+(list|get|validate|test)\\b', comment: 'grose-agent source read-only operations' },
    { regex: /^grose-agent\s+skill\s+(list|get|validate|where)\b/, source: '^grose-agent\\s+skill\\s+(list|get|validate|where)\\b', comment: 'grose-agent skill read-only operations' },
    { regex: /^grose-agent\s+automation\s+(list|get|validate|history|last-executed|test|lint)\b/, source: '^grose-agent\\s+automation\\s+(list|get|validate|history|last-executed|test|lint)\\b', comment: 'grose-agent automation read-only operations' },
    { regex: /^grose-agent\s*$/, source: '^grose-agent\\s*$', comment: 'grose-agent bare invocation' },
    { regex: /^grose-agent\s+(label|source|skill|automation)\s+--help\b/, source: '^grose-agent\\s+(label|source|skill|automation)\\s+--help\\b', comment: 'grose-agent entity help flags' },
    { regex: /^grose-agent\s+--(help|version|discover)\b/, source: '^grose-agent\\s+--(help|version|discover)\\b', comment: 'grose-agent global flags' },
...
// ============================================================
// Group 27: grose-agent CLI allowlist
// ============================================================

describe('ShellGuard corpus: grose-agent CLI allowlist', () => {
  const shouldAllow = [
    'grose-agent',
    'grose-agent --help',
    'grose-agent --version',
    'grose-agent --discover',
    'grose-agent label --help',
    'grose-agent label list',
    'grose-agent label get bug',
    'grose-agent source --help',
    'grose-agent source list',
    'grose-agent source get linear',
    'grose-agent source validate linear',
    'grose-agent source test linear',
    'grose-agent skill --help',
    'grose-agent skill list',
    'grose-agent skill get commit-helper',
    'grose-agent skill where commit-helper',
    'grose-agent skill validate commit-helper',
    'grose-agent automation list',
    'grose-agent automation get abc123',
    'grose-agent automation validate',
    'grose-agent automation history abc123 --limit 5',
    'grose-agent automation last-executed abc123',
    'grose-agent automation test abc123 --match "x"',
    'grose-agent automation lint',
  ];

  const shouldBlock = [
    'grose-agent label create --name Bug',
    'grose-agent label update bug --name "Bug Report"',
    'grose-agent label delete bug',
    'grose-agent label move bug --parent root',
    'grose-agent label reorder --parent root a b c',
    'grose-agent source create --name Linear --provider linear --type mcp',
    'grose-agent source update linear --json "{\"enabled\":false}"',
    'grose-agent source delete linear',
    'grose-agent skill create --name "Review" --description "x"',
    'grose-agent skill update review --json "{\"description\":\"y\"}"',
    'grose-agent skill delete review',
    'grose-agent automation create --event UserPromptSubmit --prompt "x"',
    'grose-agent automation update abc123 --json "{\"enabled\":false}"',
    'grose-agent automation delete abc123',
    'grose-agent automation enable abc123',
    'grose-agent automation disable abc123',
