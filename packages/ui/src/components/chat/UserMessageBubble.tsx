...
import type { StoredAttachment, ContentBadge } from '@grose-agent/core'
import { normalizePath } from '@grose-agent/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .grose-agent workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.grose-agent\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
