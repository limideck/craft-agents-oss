...
import type {
  ...
  MessagingLogMeta,
  MessagingLogger,
} from '@grose-agent/messaging-gateway'
 ...
 * 2) GROSE_IS_PACKAGED env (when explicitly set)
...
function resolveDebugMode(): boolean {
  ...
  const packagedEnv = process.env.GROSE_IS_PACKAGED
 ...
 */
export const messagingGatewayLogPath = join(homedir(), '.grose-agent', 'logs', 'messaging-gateway.log')
 ...
 */
export const autoUpdateLogPath = join(homedir(), '.grose-agent', 'logs', 'auto-update.log')
