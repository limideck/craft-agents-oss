...
import { DEFAULT_THEME, loadAppTheme, getAllowRemoteEvaluate } from '@grose-agent/shared/config'
import { CodedError } from '@grose-agent/shared/protocol'
import { getBrowserLiveFxCornerRadii } from '../shared/browser-live-fx'
import type {
  IBrowserPaneManager,
  BrowserInstanceSnapshot,
} from '@grose-agent/server-core/handlers'
import type {
...
const THEME_COLOR_SIGNAL_PREFIX = '__grose_theme_color__:'
...
const GROSE_DEEPLINK_SCHEME_PREFIX = `${process.env.GROSE_DEEPLINK_SCHEME || 'groseagents'}://`
...
export class BrowserPaneManager implements IBrowserPaneManager {
  ...
  private buildDeepLinkFromRoute(route: string): string {
    ...
    return `${GROSE_DEEPLINK_SCHEME_PREFIX}${normalizedPath}${routeQuery ? `?${routeQuery}` : ''}`
  }
  ...
  // validateUploadFilePath removed — uses shared validateFilePath from @grose-agent/server-core/handlers
...
<html>
  ...
  private async handleDeepLinkUrl(url: string): Promise<void> {
    if (!url.startsWith(GROSE_DEEPLINK_SCHEME_PREFIX)) return
...
<html>
  ...
  private installThemeObserver(instance: BrowserInstance, allowRetry = true): void {
    ...
    void instance.pageView.webContents.executeJavaScript(`
      (() => {
        ...
        const w = window;
        const previousCleanup = w.__GROSE_THEME_OBSERVER_CLEANUP__;
        ...
        w.__GROSE_THEME_OBSERVER_CLEANUP__ = () => {
  ...
  private setupWindowListeners(instance: BrowserInstance): void {
    ...
    pageWc.on('will-navigate', (event, url) => {
      if (url.startsWith(GROSE_DEEPLINK_SCHEME_PREFIX)) {
        event.preventDefault()
    ...
    pageWc.setWindowOpenHandler((details) => {
      ...
      if (details.url.startsWith(GROSE_DEEPLINK_SCHEME_PREFIX)) {
