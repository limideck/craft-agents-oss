...
async function getExpectedChannels(): Promise<Set<string>> {
  ...
  const [browser, guiSystem, guiWorkspace, guiSettings, openConnector, groseModules, tables] = await Promise.all([
    import('../browser'),
    import('../system'),
    import('../workspace'),
    import('../settings'),
...
describe('RPC handler registration', () => {
  ...
  it('keeps onboarding channels in registration coverage', async () => {
    const { HANDLED_CHANNELS } = await import('@grose-agent/server-core/handlers/rpc/onboarding')
    const { registerAllRpcHandlers } = await import('../index')
