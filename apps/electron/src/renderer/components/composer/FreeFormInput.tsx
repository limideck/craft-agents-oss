...
import { Icon_Home, Spinner } from '@grose-agent/ui'
...
import { AttachmentPreview } from '../sessions/AttachmentPreview'
...
import { type ThinkingLevel, THINKING_LEVELS, getThinkingLevelNameKey } from '@grose-agent/shared/agent/thinking-levels'
import { useEscapeInterrupt } from '@/context/EscapeInterruptContext'
import { hasOpenOverlay } from '@/lib/overlay-detection'
import { ToolbarStatusSlot } from './ToolbarStatusSlot'
import { buildPlanApprovalMessage } from '../sessions/plan-approval-message'
...
export function FreeFormInput({
  ...
  // Voice: snapshot of composer text captured when recording starts, and a flag
  // tracking whether a live interim draft is currently shown (so the stop-time
  // onResult, which repeats the same accumulated text, can be skipped).
  const voiceBaseInputRef = React.useRef('')
  const interimShownRef = React.useRef(false)
  ...
  // Listen for grose:insert-text events (generic mechanism for inserting text into input)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('grose:insert-text', handleInsertText as EventListener)
    return () => window.removeEventListener('grose:insert-text', handleInsertText as EventListener)
  ...
  // Listen for grose:approve-plan events (used by ResponseCard's Accept Plan button)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('grose:approve-plan', handleApprovePlan as EventListener)
    return () => window.removeEventListener('grose:approve-plan', handleApprovePlan as EventListener)
  }, [sessionId, permissionMode, onPermissionModeChange, onSubmit, consumeInputDraftSnapshot])

  // Listen for grose:approve-plan-with-compact events (Accept & Compact option)
  ...
  React.useEffect(() => {
    const handleApprovePlanWithCompact = async (e: CustomEvent<PlanApprovalEventDetail>) => {
      ...
      const handleCompactionComplete = async (compactEvent: CustomEvent<{ sessionId?: string }>) => {
        ...
        window.removeEventListener('grose:compaction-complete', handleCompactionComplete as unknown as EventListener)
      ...
      window.addEventListener('grose:compaction-complete', handleCompactionComplete as unknown as EventListener)
    }

    window.addEventListener('grose:approve-plan-with-compact', handleApprovePlanWithCompact as unknown as EventListener)
    return () => window.removeEventListener('grose:approve-plan-with-compact', handleApprovePlanWithCompact as unknown as EventListener)
  }, [sessionId, permissionMode, onPermissionModeChange, onSubmit, consumeInputDraftSnapshot])
  ...
  // Listen for grose:focus-input events (restore focus after popover/dropdown closes)
  React.useEffect(() => {
    ...
    window.addEventListener('grose:focus-input', handleFocusInput)
    return () => window.removeEventListener('grose:focus-input', handleFocusInput)
  ...
  // Listen for grose:paste-files events (for global paste when input not focused)
  React.useEffect(() => {
    ...
    window.addEventListener('grose:paste-files', handlePasteFiles as unknown as EventListener)
    return () => window.removeEventListener('grose:paste-files', handlePasteFiles as unknown as EventListener)
  ...
  // Listen for grose:submit-input events (simulate pressing the Send button)
  React.useEffect(() => {
    ...
    window.addEventListener('grose:submit-input', handleSubmitInput as EventListener)
    return () => window.removeEventListener('grose:submit-input', handleSubmitInput as EventListener)
  ...
  // Insert transcribed voice text at the current caret position.
  const appendVoiceText = React.useCallback(
    (text: string) => {
      // If a live interim draft is already shown, the stop-time onResult carries
      // the same accumulated text — skip it to avoid duplicating into the composer.
      if (interimShownRef.current) {
      ...
      if (!trimmed) return
      const current = richInputRef.current?.value ?? input
      const caret = richInputRef.current?.selectionStart ?? current.length
      const needsLeadingSpace = caret > 0 && !/\s$/.test(current.slice(0, caret))
      const separator = needsLeadingSpace ? ' ' : ''
      const next = current.slice(0, caret) + separator + trimmed + current.slice(caret)
      setInput(next)
      syncToParent(next)
      const newCaret = caret + separator.length + trimmed.length
      setTimeout(() => {
        richInputRef.current?.setValue(next)
        richInputRef.current?.setSelectionRange(newCaret, newCaret)
        richInputRef.current?.focus()
      }, 0)
    },
    [input, syncToParent],
  ...
  // Live partial transcript while recording — mirrors test/voice-input-app's
  // appendInt: render interim text as a draft in the composer, finalize on stop.
  const handleInterim = React.useCallback(
    (text: string) => {
      ...
      setTimeout(() => {
        ...
        richInputRef.current?.setValue(next)
        richInputRef.current?.setSelectionRange(pos, pos)
      }, 0)
    },
    [syncToParent],
  )
  ...
  const voice = useVoiceInput({
    ...
    onModelMissing: () => {
      // Surface a toast so the user knows to download the model in Settings.
      toast.error(t('voice.modelMissingToast'), {
        action: {
          label: t('voice.openSettings'),
          onClick: () => navigate(routes.view.settings('voice')),
        },
      })
    },
    onError: (code) => {
  ...
  // Snapshot composer text when a recording starts, so live interim drafts are
  // appended after it (matching test/voice-input-app's appendInt behaviour).
  React.useEffect(() => {
    if (voice.status === 'recording') {
      voiceBaseInputRef.current = richInputRef.current?.value ?? ''
    }
  }, [voice.status])
    ...
    <form onSubmit={handleSubmit}>
      ...
      >
        ...
        {/* Controlled EditPopover for "Add New Label" — opens when user selects
            ...
            Electron but launched the desktop app from the WebUI via `groseagents://`.
        ...
        <RichTextInput
          ...
          onBlur={() => {
            // Save caret position before losing focus (for restoration via grose:focus-input)
        ...
        <div className="relative">
          ...
          {/* Compact mode: permission mode drawer + standard icon badges for attach/sources/working dir.
              ...
              anchored to the right (grose-agents-oss#798). overflow-hidden is safe — Radix Drawer /
          ...
          <Tooltip delayDuration={250}>
            <TooltipTrigger asChild>
              <button
                type="button"
                onClick={() =>
                  voice.status === 'recording' ? voice.stop() : voice.start()
                }
                disabled={disabled || voice.status === 'transcribing'}
                aria-label={t('voice.mic')}
                className={cn(
                  "h-7 w-7 flex items-center justify-center rounded-[6px] transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-ring",
