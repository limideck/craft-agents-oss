...
import { Spinner, FullscreenOverlayBase, Tooltip, TooltipTrigger, TooltipContent } from '@grose-agent/ui'
import { useSetAtom } from 'jotai'
import { fullscreenOverlayOpenAtom } from '@/atoms/overlay'
import { motion, AnimatePresence } from 'motion/react'
import type { LlmConnectionWithStatus, ThinkingLevel, WorkspaceSettings, Workspace } from '../../../shared/types'
import { DEFAULT_THINKING_LEVEL, THINKING_LEVELS } from '@grose-agent/shared/agent/thinking-levels'
    ...
    switch (provider) {
      ...
      case 'pi': {
        ...
        const piLabel = !isSubscription && connection.piAuthProvider
          ...
          : null
        parts.push(piLabel ?? 'Grose Agents Backend')
        break
      }
      case 'pi_compat':
        parts.push(connection.baseUrl?.toLowerCase().includes('manifest.build')
          ? 'Manifest'
          : 'Grose Agents Backend Compatible')
        break
                ...
                options={[
                  ...
                  ...llmConnections.map((conn) => ({
                    ...
                    description: conn.providerType === 'anthropic' ? 'Anthropic' :
                                 conn.providerType === 'pi' ? 'Grose Agents Backend' :
                  ...
                  <SettingsMenuSelectRow
                    ...
                    options={llmConnections.map((conn) => ({
                      ...
                      description: conn.providerType === 'anthropic' ? 'Anthropic API' :
                                   conn.providerType === 'pi' ? 'Grose Agents Backend' :
                                   conn.providerType === 'pi_compat' ? (conn.baseUrl?.toLowerCase().includes('manifest.build') ? 'Manifest' : 'Grose Agents Backend Compatible') :
                                   conn.providerType || 'Unknown',
                    }))}
                  />
                  <SettingsMenuSelectRow
