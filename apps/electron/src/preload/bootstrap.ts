 ...
 *
 * Thin-client mode (GROSE_SERVER_URL):
...
import { createCallbackServer } from '@grose-agent/shared/auth/callback-server'
import { CHATGPT_OAUTH_CONFIG } from '@grose-agent/shared/auth/chatgpt-oauth-config'
import {
...
import type { ConfirmDialogSpec, FileDialogSpec, BrowserCapabilityRequest } from '@grose-agent/server-core/transport'
import type { RpcClient } from '@grose-agent/server-core/transport'
import type { RemoteServerConfig } from '@grose-agent/core/types'
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.GROSE_SERVER_URL!
  const wsToken = process.env.GROSE_SERVER_TOKEN ?? ''
  ...
  if (parsed.protocol === 'ws:' && !isLocalhost) {
    throw new Error(
      ...
      `Set GROSE_RPC_TLS_CERT/KEY on the server to enable TLS.`
    )
  }

  // Workspace ID is optional — if missing, renderer shows a workspace picker
  const workspaceId = process.env.GROSE_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
;(api as ElectronAPI).getSystemWarnings = async () => ({
  vcredistMissing: process.env.GROSE_VCREDIST_MISSING === '1',
  downloadUrl: process.env.GROSE_VCREDIST_URL,
})
