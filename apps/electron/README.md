# Grose Agents Electron App

The primary desktop interface for Grose Agents, built with Electron + React. Provides a multi-session inbox with chat interface for interacting with Claude via Grose workspaces.
...
│   │   ├── sessions.ts    # Session management, GroseAgent integration
...
The `AgentEvent` types from `GroseAgent` use different property names than you might expect:
...
### 4. GroseAgent Constructor

`GroseAgent` expects the full `Workspace` object, not just the ID:

```typescript
// Wrong:
new GroseAgent({ workspaceId: workspace.id, model })

// Correct:
new GroseAgent({ workspace, model })
```
...
**How it works:**
- `.env.1password` contains `op://` references to the `Dev_Grose_Agents` vault
...
- **Subagent support** - Load and apply agent definitions from Grose documents
...
```
groseagents://settings
groseagents://allSessions/session/session123
groseagents://sources/source/github
groseagents://action/new-chat
groseagents://workspace/{id}/allSessions/session/abc123
```
...
| `main/sessions.ts` | GroseAgent wrapper, event processing, source integration |
