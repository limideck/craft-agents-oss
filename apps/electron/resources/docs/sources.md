...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in Grose Agent.

> **CLI-first workflow (recommended):** Use `grose-agent source ...` commands instead of editing source config files directly.
...
**Before doing anything else**, search for a specialized guide using the grose-agents-docs MCP:

```
mcp__grose-agents-docs__SearchGroseAgents({ query: "{service} source setup" })
```

**Available guides:** GitHub, Linear, Slack, Gmail, Google Calendar, Google Drive, Google Docs, Google Sheets, Outlook, Microsoft Calendar, Teams, SharePoint, Grose, Filesystem, Brave Search, Memory
...
- "To find issues in the Grose iOS project: ..."
...
Agent: [FIRST: Searches for Linear guide]
       mcp__grose-agents-docs__SearchGroseAgents({ query: "linear source setup" })
...
User: Issue tracking for my iOS team, project called "Grose iOS"
...
Agent: [Creates config.json with:
        ...
        - tagline: "Issue tracking for the Grose iOS team"
        - OAuth authentication]
Agent: [Writes guide.md mentioning "Grose iOS" project and iOS team context]
...
Agent: Linear is now set up! I've configured it to focus on the "Grose iOS" project.
You can use it to:
- Search and view issues in Grose iOS
...
- `~/.grose-agent/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
```json
{
  ...
  "provider": "grose",
  "api": {
    "baseUrl": "https://connect.grose.do/my/api/v1/",
    "authType": "oauth"
  }
}
```
...
`~/.grose-agent/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.grose-agent/workspaces/{ws}/sources/my-source
   ```
