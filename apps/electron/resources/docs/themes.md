...
This guide explains how to customize the visual theme of Grose Agent.

## Overview

Grose Agent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.grose-agent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.grose-agent/theme.json` - Override specific colors (app-level)
...
```
~/.grose-agent/workspaces/{id}/config.json
```
...
Create `~/.grose-agent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.grose-agent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.grose-agent/themes/{name}.json`
...
1. Create `~/.grose-agent/theme.json` for overrides or `~/.grose-agent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.grose-agent/theme.json` for overrides, `~/.grose-agent/themes/` for presets)
