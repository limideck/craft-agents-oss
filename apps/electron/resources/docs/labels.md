...
> **CLI-first workflow (recommended):** Use `grose-agent label ...` commands instead of editing JSON directly.
> - `grose-agent label --help`
> - Canonical command reference: [grose-cli.md](./grose-cli.md)

## Storage Locations

- Config: `~/.grose-agent/workspaces/{id}/labels/config.json`
...
Prefer `grose-agent` commands:

```bash
grose-agent label create --name "Bug" --color "destructive"
grose-agent label create --name "Priority" --color "accent" --value-type number
grose-agent label create --name "Due Date" --color "info" --value-type date
grose-agent label create --name "Docs" --color "info" --value-type link
grose-agent label create --name "Project" --color "foreground/60"
grose-agent label create --name "Alpha" --color "info" --parent-id project
grose-agent label create --name "Beta" --color "success" --parent-id project
```
