// Package workspace resolves Craft workspace id → absolute disk rootPath.
...
const headerRoot = "X-Craft-Workspace-Root"
...
// ConfigPath returns the global Craft config.json path (registry of workspaces).
func ConfigPath() string {
	if p := strings.TrimSpace(os.Getenv("CRAFT_CONFIG_PATH")); p != "" {
		return p
	}
	...
	return filepath.Join(home, ".craft-agent", "config.json")
}
...
func lookupRegistry(workspaceID string) (string, error) {
	...
	path := ConfigPath()
	if path == "" {
		return "", fmt.Errorf("cannot resolve home for Craft config")
	}
