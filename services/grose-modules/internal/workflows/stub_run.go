...
func sampleOutput(n Node, input map[string]any) map[string]any {
	...
	case "agent":
		...
		// Direct Go/MCP path only — Grose `workflows:run` replaces this with a real session reply.
		return map[string]any{
			"text":      fmt.Sprintf("(Go stub) agent %q — execute via Grose workflows:run for a real model reply", agent),
			"model":     input["model"],
			"delegated": true,
		}
	case "generate-image":
