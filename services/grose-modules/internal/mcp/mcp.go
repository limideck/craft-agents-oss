...
func NewServer(deps Deps) *sdkmcp.Server {
	c := newClient(deps)
	server := sdkmcp.NewServer(&sdkmcp.Implementation{Name: "grose-modules", Version: config.Version}, nil)
...
type workspaceInput struct {
	WorkspaceID string `json:"workspace_id,omitempty" jsonschema:"Grose workspace id from <grose_modules> workspace_id (matches Workbench UI). Prefer omitting to use GROSE_DEFAULT_WORKSPACE_ID."`
}
...
func (c *client) request(ctx context.Context, method, path, workspaceID string, body any) (any, error) {
	if workspaceID == "" {
		return nil, fmt.Errorf("workspace_id required (tool arg or GROSE_DEFAULT_WORKSPACE_ID)")
	}
	...
	req.Header.Set("X-Grose-Workspace-Id", workspaceID)
	if body != nil {
...
func (c *client) getRaw(ctx context.Context, path, workspaceID string) (string, error) {
	if workspaceID == "" {
		return "", fmt.Errorf("workspace_id required (tool arg or GROSE_DEFAULT_WORKSPACE_ID)")
	}
