...
func List(rdb *sql.DB) ([]Site, error) {
	var out []Site
	err := grosedb.WithBusyRetry(func() error {
...
func writeGroseMeta(dir string, site Site) error {
	meta := map[string]any{
		"id":       site.ID,
		"name":     site.Name,
		"slug":     site.Slug,
	...
	return os.WriteFile(filepath.Join(dir, ".grose-sites.json"), append(b, '\n'), 0o644)
}
