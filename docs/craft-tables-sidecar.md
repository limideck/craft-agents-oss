# Craft Tables sidecar (plydb fork)

Independent Go sidecar (fork of [plydb](https://github.com/kineticloom/plydb), origin `limideck/plydb`) that Craft Agents hosts like Open Connector: spawn locally, register as one HTTP MCP Source (`tables`), and proxy Admin HTTP for the Workbench UI.

**Repo layout today:** nested clone at `test/plydb/` (gitignored under `test/`). Later: publish as its own git project; Craft only vendors the binary + thin lifecycle glue.
...
| MCP | `/mcp` | Bearer `TABLES_TOKEN` | Craft Agent (MCP Source) |
...
./plydb serve --data-dir ~/.craft-agent/workspaces/my-slug/modules/tables --addr 127.0.0.1:3911 --token dev
CRAFT_TABLES_URL=http://127.0.0.1:3911 bun run electron:dev
```
...
| Dock `tables-grid` | Breadcrumb + table chips + `@craft-agent/datagrid` readonly preview |
...
See [workspace-storage.md](./workspace-storage.md). Env: `CRAFT_TABLES_URL`, `CRAFT_TABLES_BIN`, `TABLES_TOKEN`, `TABLES_DATA_DIR`, `TABLES_ADDR`.
