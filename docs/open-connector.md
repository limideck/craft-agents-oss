...
Craft Agents hosts [open-connector](../open-connector/) as a local sidecar process and registers it as a single HTTP MCP Source (`open-connector`). Agents discover and run provider actions through four MCP tools (`list_apps`, `search_actions`, `get_action_guide`, `execute_action`) instead of installing one skill/MCP per SaaS.

## Dev setup

1. Clone open-connector next to the craft repo root (already present if you followed the integration plan):
...
2. Start Craft Electron as usual:
...
Then launch Craft with:

```bash
CRAFT_OPENCONNECTOR_URL=http://127.0.0.1:3000 bun run electron:dev
```

Tokens still come from `~/.craft-agent/open-connector/sidecar-secrets.json` (or `$CRAFT_CONFIG_DIR/open-connector/`). For an external instance, set matching `OOMOL_CONNECT_ADMIN_TOKEN` / `OOMOL_CONNECT_RUNTIME_TOKEN` / `OOMOL_CONNECT_ENCRYPTION_KEY` when starting open-connector, or copy values from the secrets file.
...
Renderer admin calls should use `@craft-agent/open-connector-client` with `baseUrl` + `adminToken` from IPC, and an IPC-backed `fetch` (`openConnectorFetch`) so catalog requests are not blocked by CORS.

## Data & secrets

| Path / env | Purpose |
|------------|---------|
| `~/.craft-agent/open-connector/` | SQLite runtime DB + sidecar secrets |
...
| `CRAFT_OPENCONNECTOR_NODE` | Optional path to Node binary (dev) |
...
Provider-specific app registration (Google Cloud Console, GitHub OAuth Apps, …) is unchanged from open-connector’s own docs — use the Craft-managed origin as the redirect base.
...
If the packaged entry is missing, the app skips spawn and logs a warning; set `CRAFT_OPENCONNECTOR_URL` for debugging packaged builds against an external runtime.
