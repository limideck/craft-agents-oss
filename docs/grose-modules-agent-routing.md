...
Status: **Phases 1 + 3 implemented** — Module Registry + `<grose_modules>` injection; per-turn workbench `activeModuleId` → PromptBuilder. Optional Skills = Phase 2.
...
**Accepted:** three-layer prefer-builtin routing — TS Module Registry → Policy Context every turn → single `grose-modules` MCP with namespaced tools. Skills optional and thin. Workbench active module enriches the same context block when available.
