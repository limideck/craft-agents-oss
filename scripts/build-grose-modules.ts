#!/usr/bin/env bun
/**
 * Build grose-modules Go sidecar and stage it for Electron packaging.
 *
 * Usage:
 *   bun run scripts/build-grose-modules.ts           # host platform → bin + electron resources
 *   bun run scripts/build-grose-modules.ts --all     # cross-compile dist/* for all targets
 *   bun run scripts/build-grose-modules.ts --stage-only  # copy existing dist/host into resources
 */
...
const svc = join(root, 'services', 'grose-modules')
const resourcesRoot = join(root, 'apps', 'electron', 'resources', 'grose-modules')
...
function binaryName(goos: string): string {
  return goos === 'windows' ? 'grose-modules.exe' : 'grose-modules'
}
...
function stageHost(): void {
  ...
  if (!existsSync(from)) {
    throw new Error(`grose-modules binary not found at ${src} or ${alt}. Run make dist-host first.`)
  }
  ...
  console.log(`[grose-modules] staged ${from} → ${flatDest}`)
  console.log(`[grose-modules] staged ${from} → ${archDest}`)
}
