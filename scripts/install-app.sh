...
DOWNLOAD_DIR="$HOME/.grose-agent/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Grose-Agents-x64.AppImage"
...
if [ -z "$filename" ]; then
    filename="Grose-Agents-${arch}.${ext}"
fi
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.lukilabs.grose-agent"
    if pgrep -x "Grose Agents" >/dev/null 2>&1; then
        info "Quitting Grose Agents..."
        ...
        if pgrep -x "Grose Agents" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Grose Agents" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Grose Agents has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Grose Agents'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.grose-agent/app"
    WRAPPER_PATH="$INSTALL_DIR/grose-agents"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Grose-Agents-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Grose-Agent.*AppImage" >/dev/null 2>&1; then
        info "Stopping Grose Agents..."
        pkill -f "Grose-Agent.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# Grose Agent launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.grose-agent/app/Grose-Agents-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@grose-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@grose-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Grose Agent not found at $APPIMAGE_PATH"
    echo "Reinstall: curl -fsSL https://agents.grose.do/install-app.sh | bash"
    exit 1
fi
...
# AppImage creates a new /tmp/.mount_Grose-XXXX each launch, so any cached path is stale
for cache_dir in "$ELECTRON_CACHE" "$ELECTRON_CACHE_ALT"; do
    if [ -d "$cache_dir" ] && grep -rq '/tmp/\.mount_Grose' "$cache_dir" 2>/dev/null; then
        rm -rf "$cache_dir"
    fi
done
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Grose-Agents-x64.AppImage"
